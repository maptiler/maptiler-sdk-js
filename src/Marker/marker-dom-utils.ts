...
//#region applyClusterCount

const CLUSTER_COUNT_CLASSNAME = "marker-cluster-count";

/**
 * Shows the member count as the *main* text of a cluster representative:
 * the marker's own content layer is hidden (never removed — image/element
 * content survives the round-trip) and a count label is rendered in the
 * shape's content geometry. Markers without a shape SVG (custom elements,
 * `xs` dots) get a centred overlay label instead.
 * @param element - The marker's outer root element (or the custom element).
 * @param count - Member count to display, or `null` to restore the marker's own content.
 */
export function applyClusterCount(element: HTMLElement, count: number | null): void {
  const wrapper = element.classList.contains(CUSTOM_ELEMENT_CLASSNAME) ? element : (element.querySelector<HTMLElement>(`.${CUSTOM_ELEMENT_CLASSNAME}`) ?? element);
  const svg = getShapeSvg(wrapper);
  const content = (svg ?? wrapper).querySelector<SVGElement>(".marker-content");

  if (count === null) {
    (svg ?? wrapper).querySelector(`.${CLUSTER_COUNT_CLASSNAME}`)?.remove();
  ...
  if (svg) {
    let text = svg.querySelector<SVGTextElement>(`text.${CLUSTER_COUNT_CLASSNAME}`);
    if (!text) {
      const shapeKey = (wrapper.dataset.markerShape ?? DEFAULT_SHAPE) as NonNullable<MapTilerMarkerBaseOptions["shape"]>;
      text = appendTextContent(svg, label, SHAPES[shapeKey], CLUSTER_COUNT_CLASSNAME);
    }
    // per-pass call (per-frame while the camera moves) — skip unchanged writes
    if (text.textContent !== label) text.textContent = label;
    return;
  }

  // no shape SVG: centred overlay label
  let overlay = wrapper.querySelector<HTMLElement>(`div.${CLUSTER_COUNT_CLASSNAME}`);
  if (!overlay) {
    overlay = document.createElement("div");
    overlay.className = CLUSTER_COUNT_CLASSNAME;
    overlay.style.position = "absolute";
    overlay.style.inset = "0";
    overlay.style.display = "flex";
    overlay.style.alignItems = "center";
    overlay.style.justifyContent = "center";
    overlay.style.font = "700 12px Inter, system-ui, sans-serif";
    overlay.style.color = "var(--marker-content-color)";
...
//#region setWrapperScale

/**
 * Writes the wrapper's 2-D scale (dataset + recomposed transform) directly.
 * Used by the cluster appearance to scale the marker without touching the
 * marker's `scale` prop.
 */
export function setWrapperScale(element: HTMLElement, sx: number, sy: number): void {
  const wrapper = element.classList.contains(CUSTOM_ELEMENT_CLASSNAME) ? element : (element.querySelector<HTMLElement>(`.${CUSTOM_ELEMENT_CLASSNAME}`) ?? element);
  wrapper.dataset.scaleX = String(sx);
  wrapper.dataset.scaleY = String(sy);
  applyTransform(wrapper);
