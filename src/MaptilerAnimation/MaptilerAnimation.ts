...
export default class MaptilerAnimation {
  ...
  private listeners: AnimationEventListenersRecord = AnimationEventTypesArray.reduce((acc, type) => {
    acc[type] = [];
    return acc;
  ...
  update(manual = true, ignoreIteration = false) {
    ...
    if (current?.id !== this.currentKeyframe) {
      this.emitEvent("keyframe", current, next);
    }
  ...
  setPlaybackRate(rate: number) {
    ...
    this.emitEvent("playbackratechange");
    return this;
  }
  ...
  destroy() {
    this.stop();
    this.listeners = AnimationEventTypesArray.reduce((acc, type) => {
      acc[type] = [];
      return acc;
    }, {} as AnimationEventListenersRecord);
