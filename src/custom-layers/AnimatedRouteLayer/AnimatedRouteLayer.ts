...
export class AnimatedRouteLayer implements CustomLayerInterface {
  ...
  private enquedEventHandlers: AnimationEventListenersRecord = AnimationEventTypesArray.reduce<AnimationEventListenersRecord>((acc, type) => {
    acc[type] = [];
    return acc;
  ...
  async onAdd(map: MapSDK): Promise<void> {
    ...
    this.animationInstance = new MaptilerAnimation({
      ...maptilerAnimationOptions,
      manualMode: this.manualUpdate,
    });

    this.animationInstance.addEventListener("timeupdate", this.update);
